...
public class Exercise6_20 {
	...
	public static int findPosition(int k, int[] queens) {
		//start at 0 if k row is empty, or after k row if it is full: this will overwrite the previous row and column if it lead to a dead end
	...
	public static boolean isValid(int k, int j, int queens[]) {
		...
		// Check jth column
		for (int i = 0; i < k; i++)// checks if there is a queen in the j column until the current row inclusive
			if (queens[i] == j)
				return false;
